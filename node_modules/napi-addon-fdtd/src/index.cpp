...
Napi::Value GetData2D(const Napi::CallbackInfo &info) 
{
  ...
  for (int i = 0; i < epsilon_vector_size; i++) 
  {
    tmp_vector_sigma.push_back(
          (float)sigma_vector_js[i].As<Napi::Number>());
  }
  ...
  {
    float source_position_relative = static_cast<float>(relative_source_position_array[i].As<Napi::Number>());
  ...
  // static FDTD_2D_UPDATED fdtd = FDTD_2D_UPDATED(lambda, tau, epsilon_vector, sigma_vector, source_position_vector);
